...
class SupplyPage extends React.Component {
    ...
    createCheckboxes = () => (
        regularItems.map(this.createCheckbox)
    );

    createAddedBoxes= () => (
        this.addedItems.map(this.createCheckbox)
    );
    ...
    handleSubmit = () => {
        ...
        dateEntry.update({
            [this.today]: {
                Supply: tempObject,
                Headcount: {totalPeople: 0, peopleAboutResource: 0, bedsCalled: 0, youth: 0}
            },
        });
                ...
                <div className="row">
                    <div className="col-sm-12">
                        ...
                        <form onSubmit={this.handleFormSubmit}>
                            ...
                            <label>
                                New Item:
                                <input type="text" name="newItem" onChange={this.handleChange} />
                            </label>
                            <button className="btn btn-default" type="add" onClick={() => this.handleAddItem()}>Add Supply</button>


                            <div><button className="btn btn-default" type="submit" onClick={() => this.handleSubmit()}>Save</button></div>
                        </form>
